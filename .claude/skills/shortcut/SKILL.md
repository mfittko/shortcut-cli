...
Missing token → exit code **11** on every command (including `--help`). If `short` is not on PATH, run `node build/bin/short.js` from the repo (after `pnpm build`) — same interface.
